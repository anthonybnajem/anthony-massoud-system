...
import { SearchBar } from "../sales/components/SearchBar";
import { CategoryFilter } from "../sales/components/CategoryFilter";
import { ProductTabs } from "../sales/components/ProductTabs";
import { ProductCard } from "../sales/components/ProductCard";
import { Cart } from "../sales/components/Cart";
import { MobileCartButton } from "../sales/components/MobileCartButton";
import {
  CustomLineItemDialog,
  type CustomLineDraft,
} from "../sales/components/CustomLineItemDialog";
import { useBarcodeScanner } from "../sales/hooks/useBarcodeScanner";
import { WeightQuantityDialog } from "../sales/components/WeightQuantityDialog";
import { VariationSelectionDialog } from "../sales/components/VariationSelectionDialog";
...
export default function ExpensesPage() {
  ...
  const [selectedEmployeeId, setSelectedEmployeeId] =
    useState<string>("");
  const [weightDialogProduct, setWeightDialogProduct] =
  ...
  >({});
  const [isCustomLineDialogOpen, setIsCustomLineDialogOpen] =
    useState(false);
  const [customLineEditingId, setCustomLineEditingId] = useState<
    string | null
  >(null);
  ...
  const cartSubtotal = cart.reduce(
    (total, item) =>
      total + getCartLineUnitPrice(item) * item.quantity,
    0,
  );
    ...
    if (!selectedDiscount) {
      const manualBase =
        discountType === "percentage"
          ...
          : discountValue;
      const manualAmount = Math.min(
        Math.max(manualBase, 0),
        cartSubtotal,
      );
      return {
        amount: manualAmount,
    ...
    if (selectedDiscount.appliesTo === "product") {
      ...
      cart.forEach((item) => {
        ...
        if (
          ...
          selectedDiscount.categoryIds?.includes(
            item.product.categoryId,
          )
        ) {
          applyLineDiscount(
  ...
  // Calculate tax
  const taxAmount =
    (taxableSubtotal - discountAmount) * (taxRate / 100);
  ...
  const removeFromCart = (lineId: string) => {
    setCart((prevCart) =>
      prevCart.filter((item) => item.id !== lineId),
    );
  };
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      await recordExpense({
        items: expenseItems,
        total: cartTotal > 0 ? -Math.abs(cartTotal) : cartTotal,
        paymentMethod,
  ...
  const handleVariationConfirm = (
    ...
    if (!variation) return;
    const linePrice =
      typeof variationDialogProduct.cost === "number"
    ...
    if (selectedDiscount) {
      if (savedDiscountError) {
        ...
        toast({
          ...
          description: `${selectedDiscount.name} is applied to this expense.`,
        });
      }
      return;
    }
  ...
  const saveNotes = () => {
    ...
    if (saleNotes.trim()) {
      toast({
        title: "Notes Saved",
        description: "Your notes have been saved to this expense.",
      });
    }
  };
  ...
  const saveCustomerInfo = () => {
    ...
    if (
      ...
      toast({
        ...
        description:
          "Customer information has been saved to this expense.",
      });
    }
  };
  ...
  return (
    ...
    >
                  ...
                  <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-green-500/10 border border-green-500/20">
                    ...
                    <span className="text-xs font-medium text-green-700 dark:text-green-400">
                      {activeShifts[0].employeeId
                        ? employees.find(
                            (e) =>
                              e.id === activeShifts[0].employeeId,
                          )?.name || "Shift Active"
                        : "Shift Active"}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col items-center justify-center h-full min-h-[300px]">
                    <Empty>
                      <EmptyHeader>
                        ...
                        <EmptyDescription>
                          You'll be able to mark products as favorites
                          for quick access
                        </EmptyDescription>
                      </EmptyHeader>
                    </Empty>
      ...
      <CustomerDialog
        ...
        onSaveProfile={async ({
          ...
          toast({
            ...
            description: `${profile.name} is now available for future expenses.`,
          });
        }}
      />
      ...
      <CustomLineItemDialog
        ...
        title={
          customLineEditingId ? "Edit Custom Line" : "Add Custom Item"
        }
        workers={workers}
